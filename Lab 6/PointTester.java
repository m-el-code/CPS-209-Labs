...
public class PointTester
{
    ...
    public static double getDistance(Point p1, Point p2)
    {
        ...
        double xDistance = Math.pow((p1.getX() - p2.getX()), 2.0);
		double yDistance = Math.pow((p1.getY() - p2.getY()), 2.0);
    ...
    public static double getMaxDistance(ArrayList<Point> points)
    {
		...
		for (int i = 0; i < points.size()- 1; i++){
			double newMaxDistance = getDistance(points.get(i), points.get(i+1));
    ...
    public static ArrayList<Point> getUniquePoints(ArrayList<Point> points)
    {
		...
		for (int i = 0; i < points.size(); i++){
			if (uniquePoints.contains(points.get(i))){
				continue;
			}
			else{	
